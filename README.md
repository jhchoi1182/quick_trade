...
- 자동·섀도: 1·3·5·15분 가격·거래량을 5분마다 종합해 돌파·이탈·지지 반등·저항 반락을 판단하는 클라이언트 OCO
...
| 자동 | `gpt-5.6-sol` 전문 스캘퍼형 진입 판단과 KIS 실주문 |
| 섀도 | 같은 OCO 판단을 실시간 호가·체결로 가상 실행. 수동 실주문은 병행 가능 |

브로커 연결은 KIS 실전만 지원한다. 데모·모의투자 런타임은 없다.

자동 판단은 15·5분봉에서 시장 국면과 다음 반대 레벨까지의 공간을 먼저 확인하고,
3·1분봉의 반복 시험·꼬리·거래량 소진/확대·압축 후 움직임으로 진입을 구체화한다.
추세 지속(`상승 돌파`/`하락 이탈`)과 반전(`지지 반등`/`저항 반락`)을 모두 지원하며,
시간축 충돌·박스 중단·추격 진입·유동성 부족처럼 우위가 불명확하면 시나리오를 만들지 않고 SKIP한다.
