...
- **UI 전용 필드**(opacity, chart_interval, theme, buffer_ticks, 라벨) 변경 → 재시작 없이 `Engine::update_settings`로 반영만
...
### 주문 규약 (사용자 확정 설계)
- **매수**: 매도1호가 + buffer_ticks IOC지정가, 예수금 기준 최대 수량(`util.rs`). 미체결 잔량은 거래소가 즉시 자동취소 → 호가창에 안 쌓임. **모의투자는 IOC 미지원이라 일반 지정가로 대체**(`engine.rs`에서 `mode==Paper`면 `ioc=false`).
- **매도**: 보유 전량 시장가.
...
| | 시세 TR | 시장 구분 | 주문 거래소 |
|---|---|---|---|
| Real | H0UN~ (KRX+NXT 통합) | UN (프리 08시~애프터 20시) | 설정값 KRX/SOR |
| Paper | H0ST~ (KRX) | J (09:00~15:30) | KRX 강제 |
| Demo | mock | — | — |

당일·과거 분봉 모두 통합(UN) 지원이 불확실해 실패 시 KRX(J)로 폴백한다(`quote.rs`).
...
3. **모의투자 수수료율** — 최대수량 매수가 "주문가능금액 부족"으로 거절되면 `util.rs::max_buy_qty`의 여유 마진(0.999)을 낮춰라(= 여유분 확대).
4. **PC 시계 동기화** — 시세 신선도 판정(`engine.rs::QUOTE_FRESH_SECS`)이 거래소 체결시각과 로컬 시계를 비교하므로, 시계가 수십 초 틀어지면 주문마다 불필요한 REST 스냅샷 폴백이 발생한다.
5. **웹소켓 구독 실패 로그** — 접속은 됐는데 시세가 없으면 "웹소켓 구독 실패" 경고 로그부터 확인(`ws.rs`).
