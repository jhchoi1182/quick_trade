...
5. **웹소켓 구독 거절 로그** — 접속은 됐는데 시세가 없으면 "웹소켓 구독 거절"/"미확인" 경고 로그부터 확인(`ws.rs`). 부분 구독 실패는 연결을 유지한 채 30초 주기로 해당 구독만 재시도하고("웹소켓 재구독 성공" info 로그로 회복 확인, 이미 구독된 경우 ALREADY IN SUBSCRIBE 응답이 성공 처리됨), 성공 ack·데이터가 전혀 없을 때만 재접속(매 시도 접속키 신규 발급)한다. 종목별로 "실시간 체결가 첫 수신" info 로그(`engine.rs::consume_feed`)가 찍히는지 보면 어떤 코드가 실제 시세를 받는지 즉시 판별된다.
