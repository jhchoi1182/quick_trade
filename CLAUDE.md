...
- `automationStore.ts`/`AutomationPanel.tsx`: 5초 `get_automation_status` 폴링 + `automation-state` emit을 함께 받는다. 이벤트가 기본 갱신 경로이고 폴링은 누락 복구용이다. `keepNewest`가 `runtimeGeneration` 다음 `revision` 순으로 비교해, 이벤트 순서가 뒤바뀌어도 오래된 스냅샷이 최신 상태를 덮어쓰지 않게 한다. `automationPresentation.ts`는 phase/시나리오 상태를 화면 문구로 변환하고, `llmUsage.ts`는 OpenAI 토큰 사용량·비용을 계산한다.
...
5. **웹소켓 구독 거절 로그** — 접속은 됐는데 시세가 없으면 "웹소켓 구독 거절"/"미확인" 경고 로그부터 확인(`ws.rs`). 부분 구독 실패는 연결을 유지한 채 30초 주기로 해당 구독만 재시도하고("웹소켓 재구독 성공" info 로그로 회복 확인, 이미 구독된 경우 ALREADY IN SUBSCRIBE 응답이 성공 처리됨), 성공 ack·데이터가 전혀 없을 때만 재접속(매 시도 접속키 신규 발급)한다. 재구독이 10회(약 5분)를 넘겨도 미확인이면 전체 재접속으로 승격하고, ack는 성공인데 데이터가 안 오는 반사망 구독(체결은 신선한데 호가 미수신 60초 지속 등)은 엔진 감시견(`engine.rs::feed_watchdog_tick`)이 전체 재접속을 요청한다(5분당 1회 제한). 종목별로 "실시간 체결가 첫 수신"/"실시간 호가 첫 수신" info 로그(`engine.rs::consume_feed`)가 찍히는지 보면 어떤 코드가 실제 시세를 받는지 즉시 판별된다. 로그는 stdout과 함께 앱 데이터 폴더 `logs/easy-scalping.log.YYYY-MM-DD`(7일 보존)에 남으므로 설치본 실행도 사후 진단 가능하다.
...
7. **웹소켓 세션은 app key당 1개** — 두 번째 접속은 KIS가 OPSP8996 "ALREADY IN USE appkey" 응답 후 즉시 리셋한다(2026-07-24 프로브 실측, 접속키를 새로 발급해도 무관). 개발 실행(`npm run tauri dev`)과 설치본을 동시에 띄우면 나중에 붙은 쪽은 시세를 전혀 못 받고 백오프 재접속만 반복한다 — 로그의 "같은 app key의 웹소켓 세션이 이미 존재" 오류가 그 신호다. 구독·시세 수신을 앱 밖에서 실측하려면 앱을 종료한 뒤 `cd src-tauri && cargo run --example ws_probe`를 쓴다.
